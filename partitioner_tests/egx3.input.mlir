// tmem hoisted outside loop, token should be in partition 1
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @matmul_tma_persistent_ws_kernel(%a_ptr: !tt.ptr<f8E4M3FN> {tt.divisibility = 16 : i32}, %b_ptr: !tt.ptr<f8E4M3FN> {tt.divisibility = 16 : i32}, %c_ptr: !tt.ptr<f8E4M3FN> {tt.divisibility = 16 : i32}, %a_stride0: i32 {tt.divisibility = 16 : i32}, %b_stride0: i32 {tt.divisibility = 16 : i32}, %c_stride0: i32 {tt.divisibility = 16 : i32}, %M: i32 {tt.divisibility = 16 : i32}, %N: i32 {tt.divisibility = 16 : i32}, %K: i32 {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    %a_desc = arith.extsi %a_stride0 : i32 to i64
    %a_desc_0 = tt.make_tensor_descriptor %a_ptr, [%M, %K], [%a_desc, %c1_i64] : <f8E4M3FN>, <tensor<128x128xf8E4M3FN, #shared>>
    %b_desc = arith.extsi %b_stride0 : i32 to i64
    %b_desc_1 = tt.make_tensor_descriptor %b_ptr, [%N, %K], [%b_desc, %c1_i64] : <f8E4M3FN>, <tensor<128x128xf8E4M3FN, #shared>>
    %c_desc = arith.extsi %c_stride0 : i32 to i64
    %c_desc_2 = tt.make_tensor_descriptor %c_ptr, [%M, %N], [%c_desc, %c1_i64] : <f8E4M3FN>, <tensor<128x128xf8E4M3FN, #shared>>
    %start_pid = tt.get_program_id x : i32
    %num_pid_m = arith.addi %M, %c127_i32 : i32
    %num_pid_m_3 = arith.divsi %num_pid_m, %c128_i32 : i32
    %num_pid_n = arith.addi %N, %c127_i32 : i32
    %num_pid_n_4 = arith.divsi %num_pid_n, %c128_i32 : i32
    %k_tiles = arith.addi %K, %c127_i32 : i32
    %k_tiles_5 = arith.divsi %k_tiles, %c128_i32 : i32
    %num_tiles = arith.muli %num_pid_m_3, %num_pid_n_4 : i32
    %num_pid_in_group = arith.muli %num_pid_n_4, %c8_i32 : i32
    %accumulator, %accumulator_8 = ttng.tmem_alloc : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
    %accumulator_9 = ttng.tmem_store %cst, %accumulator[%accumulator_8], %true : tensor<128x128xf32, #blocked> -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
    %tok = scf.for %tile_id = %start_pid to %num_tiles step %c148_i32 iter_args(%accumulator_10 = %accumulator_9) -> !ttg.async.token : i32 {
      ...
      scf.yield %accumulator_13 : !ttg.async.token
